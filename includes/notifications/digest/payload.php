 ...
 */
function alpaistr_get_notification_item_rows_for_user_window( $user_id, $window_start_gmt, $window_end_gmt ) {
	global $wpdb;
 ...
 */
function alpaistr_get_notification_item_event_from_row_fallback( $row ) {
 ...
 */
function alpaistr_get_notification_item_events_for_user_window( $user_id, $window_start_gmt, $window_end_gmt ) {
	$rows   = alpaistr_get_notification_item_rows_for_user_window( $user_id, $window_start_gmt, $window_end_gmt );
	$events = [];

	foreach ( $rows as $row ) {
		$snapshot = alpaistr_get_notification_item_snapshot_from_row( $row );
		if ( empty( $snapshot ) ) {
			$snapshot = alpaistr_get_notification_item_event_from_row_fallback( $row );
		}
 ...
 */
function alpaistr_get_notification_digest_activity_priority_order() {
	return [
		'human_comments',
		'status_changes',
 ...
 */
function alpaistr_get_notification_digest_activity_priority( $event_family ) {
	$order = alpaistr_get_notification_digest_activity_priority_order();
 ...
 */
function alpaistr_get_notification_digest_plain_mentions( $text, $mentions ) {
 ...
 */
function alpaistr_get_notification_digest_comment_excerpt( $text, $mentions = [], $limit = 180 ) {
	$text = alpaistr_get_notification_digest_plain_mentions( $text, $mentions );
 ...
 */
function alpaistr_get_notification_digest_issue_status_label( $issue_id ) {
 ...
 */
function alpaistr_get_notification_digest_issue_assignees( $issue_id ) {
	...
	foreach ( $terms as $term ) {
		...
		if ( $user instanceof WP_User ) {
			$assignees[] = [
				...
				'avatar' => alpaistr_avatar( $user->ID, 24 ),
			];
			continue;
		}
 ...
 */
function alpaistr_get_notification_digest_issue_labels( $issue_id ) {
 ...
 */
function alpaistr_get_notification_digest_issue_deadline_meta( $issue_id ) {
 ...
 */
function alpaistr_get_notification_digest_issue_meta( $issue_id ) {
	$issue_id      = absint( $issue_id );
	$status_label  = alpaistr_get_notification_digest_issue_status_label( $issue_id );
	$assignees     = alpaistr_get_notification_digest_issue_assignees( $issue_id );
	$labels        = alpaistr_get_notification_digest_issue_labels( $issue_id );
	$label_names   = wp_list_pluck( $labels, 'name' );
	$deadline_meta = alpaistr_get_notification_digest_issue_deadline_meta( $issue_id );
	...
	return [
		...
		'is_high_priority' => alpaistr_is_issue_high_priority( $issue_id ),
	];
}
 ...
 */
function alpaistr_get_notification_digest_event_entry( $event ) {
	...
	$excerpt      = 'human_comments' === $event_family ? alpaistr_get_notification_digest_comment_excerpt( $comment_raw, $mentions ) : '';
	...
	return [
		...
		'priority'     => alpaistr_get_notification_digest_activity_priority( $event_family ),
	];
}
 ...
 */
function alpaistr_is_notification_digest_issue_activity_event( $event ) {
 ...
 */
function alpaistr_is_notification_digest_new_item_event( $event ) {
	$subjects = isset( $event['recipient_subjects'] ) && is_array( $event['recipient_subjects'] ) ? $event['recipient_subjects'] : [];
	$subjects = array_values( array_unique( array_filter( array_map( 'sanitize_key', $subjects ) ) ) );

	return in_array( 'all_new_tasks', $subjects, true ) && alpaistr_is_notification_new_task_event( $event );
}
 ...
 */
function alpaistr_get_notification_digest_issue_activity_groups( $events ) {
	$groups = [];

	foreach ( $events as $event ) {
		if ( ! alpaistr_is_notification_digest_issue_activity_event( $event ) ) {
			continue;
		}
		...
		if ( ! isset( $groups[ $issue_id ] ) ) {
			$groups[ $issue_id ] = [
				'issue'   => [
					...
					'meta'  => alpaistr_get_notification_digest_issue_meta( $issue_id ),
				],
				'entries' => [],
				'total'   => 0,
				'latest'  => '',
		...
		$entry                            = alpaistr_get_notification_digest_event_entry( $event );
 ...
 */
function alpaistr_get_notification_digest_done_status_id() {
	$statuses = alpaistr_get_statuses( 'DESC' );
	if ( empty( $statuses ) ) {
		return 0;
	}
 ...
 */
function alpaistr_get_notification_digest_due_issue_ids( $user_id, $preferences ) {
	...
	if ( ! empty( $preferences['subjects']['created'] ) ) {
		...
		$issue_ids         = array_merge( $issue_ids, alpaistr_to_int_ids( $created_issue_ids ) );
	}
	...
	if ( ! empty( $preferences['subjects']['assigned'] ) ) {
		...
		$issue_ids = array_merge( $issue_ids, alpaistr_to_int_ids( $assigned_issue_ids ) );
	}

	if ( ! empty( $preferences['subjects']['starred'] ) ) {
		$issue_ids = array_merge( $issue_ids, alpaistr_get_watched_issue_ids_for_user( $user_id ) );
	}
 ...
 */
function alpaistr_get_notification_deadline_watch_items( $user_id, $preferences, $window_end_gmt ) {
	$done_status_id = alpaistr_get_notification_digest_done_status_id();
	$timezone       = wp_timezone();
	...
	$issue_ids      = alpaistr_get_notification_digest_due_issue_ids( $user_id, $preferences );
	...
	foreach ( $posts as $post ) {
		...
		$meta = alpaistr_get_notification_digest_issue_meta( $post->ID );

		$items[] = [
			'id'             => (int) $post->ID,
			'title'          => (string) $post->post_title,
			'slug'           => (string) $post->post_name,
			'url'            => alpaistr_get_notification_issue_url( $post ),
 ...
 */
function alpaistr_get_notification_digest_new_items( $events, $issue_ids_in_groups ) {
	$issue_ids_in_groups = array_values( array_unique( array_filter( array_map( 'absint', $issue_ids_in_groups ) ) ) );
	$new_items           = [];

	foreach ( $events as $event ) {
		if ( ! alpaistr_is_notification_digest_new_item_event( $event ) ) {
			continue;
		}
 ...
 */
function alpaistr_get_notification_daily_digest_day_label( $window_end_gmt ) {
	$window_end = new DateTimeImmutable( $window_end_gmt, new DateTimeZone( 'UTC' ) );
 ...
 */
function alpaistr_build_notification_daily_digest_payload( $user_id, $preferences, $window_start_gmt, $window_end_gmt ) {

	$events         = alpaistr_get_notification_item_events_for_user_window( $user_id, $window_start_gmt, $window_end_gmt );
	$issue_activity = alpaistr_get_notification_digest_issue_activity_groups( $events );
	$issue_ids      = [];
	...
	$payload                        = [
		...
		'digest_day'       => alpaistr_get_notification_daily_digest_day_label( $window_end_gmt ),
		'deadline_watch'   => alpaistr_get_notification_deadline_watch_items( $user_id, $preferences, $window_end_gmt ),
	...
	if ( ! empty( $preferences['subjects']['all_new_tasks'] ) ) {
		$payload['new_items']           = alpaistr_get_notification_digest_new_items( $events, $issue_ids );
