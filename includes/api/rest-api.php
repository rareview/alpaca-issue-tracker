...
function alpaca_proxy_allowed_image_types() {
	return array( 'image/jpeg', 'image/png', 'image/gif', 'image/webp', 'image/avif', 'image/bmp', 'image/tiff' );
}
...
function alpaca_image_proxy_callback( WP_REST_Request $request ) {
	...
	if ( is_wp_error( $resp ) ) {
		return alpaca_rest_response( 'image_proxy', array( 'success' => false, 'message' => esc_html( $resp->get_error_message() ) ), 502 );
	}
	...
	if ( ! in_array( $mime_only, alpaca_proxy_allowed_image_types(), true ) ) {
		return alpaca_rest_response( 'image_proxy', array( 'success' => false, 'message' => esc_html__( 'Unsupported image type.', 'alpaca' ) ), 400 );
	}
