...
Well-scoped issues with clear acceptance criteria, file references, and bounded scope can be executed end-to-end by AI coding agents. Your job is to identify issues in the backlog that are close to that bar but haven't been labeled yet.
...
| References a specific file, class, block, or hook by name | +2 | Mentions a file path, a named class/function, a REST route, a block name, or a specific WP hook |
| Has testable acceptance criteria | +2 | Contains `- [ ]` checkbox items that describe verifiable outcomes |
| Is clearly a code bug (not content or design judgment) | +1 | Mentions errors, exceptions, PHP warnings, broken functionality, or regression |
| Has reproduction steps or error logs | +1 | Contains numbered steps, a PHP/JS error, or a code block with logs |
...
[List any file paths, class names, hooks, or block names mentioned. If none, write "Not specified."]
...
[Note any packages, APIs, or external systems mentioned]

**Testing approach:**

- Run `npm run lint` to verify coding standards
- Run `npm run build` to verify compilation
- [Add manual testing steps where appropriate]
