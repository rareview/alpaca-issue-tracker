...
function hide_comment_type( $type = '', $exclude_from_count = true ) {
	...
	$should_hide = function () {
		...
		if ( isset( $_GET['show_hidden_comments'] ) && '1' === $_GET['show_hidden_comments'] ) {
			return false;
		}

		// If this is a REST request and the param is passed, skip hiding.
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended
		if ( defined( 'REST_REQUEST' ) && REST_REQUEST && isset( $_REQUEST['show_hidden_comments'] ) && '1' === $_REQUEST['show_hidden_comments'] ) {
			return false;
		}
	...
	add_action(
		...
		function ( $query ) use ( $type, $should_hide ) {
			global $pagenow;
			if ( $should_hide() && ( ! isset( $query->query_vars['type'] ) || $type !== $query->query_vars['type'] ) ) {
				$query->query_vars['type__not_in'] = array( $type );
			}
		}
	);
	...
	add_filter(
		'comments_clauses',
		function ( $clauses ) use ( $type, $should_hide ) {
